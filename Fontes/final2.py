...
import PySimpleGUIQt as sg    #importa biblio da interface
import imaplib
import email
...
cfg_so_processa = conteudo_configuracoes[12]

email_user = cfg_email.split(',')
email_pass = cfg_senha.split(',')
#print(len(email_user))
last_id = []
for t in range (0, len(email_user)):
   last_id.append (0)
#print (last_id)

#------------------------------------------------------Parte arquivo Config. Email---------------------------------
...
cfg_mandar = conteudo_configuracoes_mail[5]
...
loadbase64 = b''
sg.theme('DarkBrown1')
...
frame_layout = [
    [sg.Checkbox('Somente Processar pasta de arquivos (vai ler de:  Pasta Temp. e salvar em: Pasta)', default= ast.literal_eval(cfg_so_processa), key='so_processa')],
...
frame_layout_2 = [
    [sg.Checkbox('Mandar E-mail Reportando Erros', default= ast.literal_eval(cfg_mandar), key='manda_mail')],
    [sg.Text('E-mail de serviço:', size=(25,1)),sg.InputText(default_text=cfg_email_srv,size=(30,1),key='cfg13')],
    [sg.Text('Senha do E-mail de serviço:', size=(25,1)),sg.InputText(default_text=cfg_senha_srv,size=(30,1),key='cfg14')],
    [sg.Text('Servidor SMTPS do E-mail de serviço:', size=(25,1)),sg.InputText(default_text=cfg_smtps_srv,size=(30,1),key='cfg15')],
    [sg.Text('Porta do E-mail de serviço:', size=(25,1)),sg.InputText(default_text=cfg_porta_srv,size=(30,1),key='cfg16')],
    [sg.Text('E-mail destino:', size=(25,1)),sg.InputText(default_text=cfg_email_dst,size=(30,1),key='cfg17')],
    [sg.Button('Testar E-mail'),sg.Button('Salvar Dados')]
    ]

layout = [
          [sg.TabGroup([[sg.Tab('Configurações', frame_layout),sg.Tab('Extras', frame_layout_2)]])]
         ]


window = sg.Window('Z-Notas XML',icon=logo).Layout(layout)

layout2 = [[sg.Button('',image_data=loadbase64,
              button_color=(sg.theme_background_color(),sg.theme_background_color()), border_width=0)],
              [sg.Button('Rodar'),sg.Button('Configurar')]
          ]

window2 = sg.Window('Bem vindo ao Z-Notas !', size=(295, 100), icon='icone.ico').Layout(layout2)


window_visible = False
window_closed = True
window2_visible = True
window2_closed = False
tray_visible = False
...
def checa_emails():
    ...
    for i in range (0, len(email_user)):
        ...
        #except:
           # sg.popup('Algo deu errado na conexao com servidor de e-mail')
        try:
    ...
    for f2 in files:
       ...
       # if f2.lower().endswith('.xml'):
        if len(f2) != (int(cfg_tamanho_xml) + 4):
            ...
            try:      
                os.rename(cfg_pasta_temp + f2, cfg_pasta_temp + f2[:-4] + '.xml')
            except:
                shutil.move(cfg_pasta_temp + f2, cfg_pasta_erro + f2)
...
def ajuste_final_erro():
    ...
    try:
        ...
        sg.popup('A pasta de destino ou origem não existe, rode novamente e altere estas informações')
        exit()
    for f2 in files:
...
#----------------------------------------------------------------------------------------------------------
    ...
    if window2_visible:
        ...
        if event == 'Rodar':  
            tray.UnHide()
            tray_visible = True
            window2_visible = False
            window2.close()
        if event == 'Configurar':
            print('Entrou em configurar')
            window2_visible = False
            window2.close()
            window_visible = True
            window_closed = False
    ...
    if window_visible:
        ...
        if event is None:
            ...
            window_closed = True
             ...
             try:
                 if cfg_email and cfg_senha and cfg_dias and cfg_porta and cfg_pasta and cfg_pasta_temp and cfg_limpeza_log and cfg_tamanho_xml and cfg_tempo and cfg_server != ' ':
                     ...
                     configuracoes.write(str(values['so_processa']) +"\n")
                     configuracoes.close()
            ...
            ajuste_final_erro()
            move_tudo()
        elif event == 'Testar E-mail':
           # print(values['cfg13'])
            config.email_sender= values['cfg13']
            config.email_senha= values['cfg14']
            config.email_smtps= values['cfg15']
            config.email_porta= values['cfg16']
            config.email_dst= values['cfg17']
            zmail.world_new()
        elif event == 'Salvar Dados':
            sg.popup('Alterações feitas execute novamente')
            configuracoes_mail = open('config_mail.txt','w')
            configuracoes_mail.write(values['cfg13'] +"\n")
            configuracoes_mail.write(values['cfg14'] +"\n")
            configuracoes_mail.write(values['cfg15'] +"\n")
            configuracoes_mail.write(values['cfg16'] +"\n")
            configuracoes_mail.write(values['cfg17'] +"\n")
            configuracoes_mail.write(str(values['manda_mail']) +"\n")
            configuracoes_mail.close()
            break
    ...
    if tray_visible:
        ...
        if dif > int(cfg_tempo2) :
           # threading.Thread(target=checa_emails, daemon=True).start()
            ...
            #print (data_exe.strftime('%d'))
            if datetime.today().strftime('%d') > data_exe.strftime('%d') :
                data_log()
            if (cfg_so_processa == 'True'):
                 print("somente processa pasta")
                 ajuste_final()
                 move_tudo()
            else:
                checa_emails()
        ...
        if menu_item == 'Configurações' or menu_item == sg.EVENT_SYSTEM_TRAY_ICON_DOUBLE_CLICKED:
            ...
            sg.popup('Sobre o Programa', 'Version 1.8 -Beta', 'Reportar problemas no endereco: duarte936@gmail.com')
